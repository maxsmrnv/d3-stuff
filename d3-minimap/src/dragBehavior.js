...
function appendClone(duplicate) {
  ...
  minimapBackground = d3
    .select('rect#minimapBackground')
...
function jumpToCursor() {
  ...
  // Right Boundary
  if (
    selectedX + minimapViewerDimensions.width * inverseK >
    minimapDimensions.width
  ) {
    selectedX =
      minimapDimensions.width - minimapViewerDimensions.width * inverseK;
  }

  // Bottom Boundary
  if (
    selectedY + minimapViewerDimensions.height * inverseK >
    minimapDimensions.height
  ) {
    selectedY =
      minimapDimensions.height - minimapViewerDimensions.height * inverseK;
  }
  ...
  minimapViewer.attr(
    'transform',
    'translate(' + selectedX + ',' + selectedY + ') scale(' + inverseK + ')'
  );
  mainViewPort.attr(
    'transform',
    'translate(' + translateX + ',' + translateY + ') scale(' + translateK + ')'
  );
  zoom.transform(
    home,
...
document.getElementById('home').addEventListener('mousemove', () => {
  /*empty*/
});
...
function zoomed() {
  ...
  viewportBoundaries = document
    .getElementById('homeVP')
    .getBoundingClientRect();
  minimapBoundaries = document
